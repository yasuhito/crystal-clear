...
- In a Japanese rewrite, preserve the source's level of deference instead of adding honorific scaffolding. Express ordinary business politeness in the request predicate: rewrite `田中様に…承認をお願いします` as a direct respectful request such as `田中様には…ご承認をお願いいたします`, not `田中様におかれましては`, unless the source already uses that ceremonial form or the user requests a more formal register.
