...
- the stated distinction between fact, inference, recommendation, and unknown, claim by claim: uncertainty in a cause or impact must not spread to a separate fixed commitment. Keep `次回更新は18時です` definite; do not weaken it to `18時を予定しております` merely because the incident cause remains uncertain;
...
- If it constrains a name in the described product or domain, it is artifact content: preserve the requirement explicitly. UI labels, API field names, schema names, and legal names belong here. “Keep the UI name X” must remain a UI-name requirement.
